...
function createAdjancyMatrix(graphData){
    ...
    console.debug("Adjancy Matrix ", adj)
    return adj;
}
...
function sameLvlPositioning(graphData, dimensions, root){
    ...
    var queue = [];
    var lvlHeight = 0, childrenCount = 0;
    if (root.length == 1){
        childrenCount = adj[root[0]].length
        if (childrenCount != 0){
    ...
    while (queue.length != 0){
        ...
        for (var z = 0; z < qsize; z++){
            ...
            childrenCount = adj[parent].length;
            ...
            for (var zz = 0;zz < adj[cur].length; zz++){
                queue.push({"node" : adj[cur][zz], "parent" : cur})
            }
    ...
    for (i = 0;i < graphData.nodes.length; i++){
        graphData.nodes[i].position = pos[i];
        graphData.nodes[i].positionAbsolute = pos[i ];
    }
